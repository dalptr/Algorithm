...
public:
    ...
    int operator%(int v) const {
        if (v < 0) v = -v;
        int m = 0;
        for (int i = z.size() - 1; i >= 0; --i) m = (z[i] + m * (i64) base) % v;
        return m * sign;
    }
    ...
    bool operator<(const INTEGER &v) const {
        ...
        if (z.size() != v.z.size()) return int(z.size()) * sign < int(v.z.size()) * v.sign;
    ...
    friend INTEGER gcd(const INTEGER &a, const INTEGER &b) { return b.is_zero() ? a : gcd(b, a % b); }

    [[maybe_unused]] friend INTEGER lcm(const INTEGER &a, const INTEGER &b) { return a / gcd(a, b) * b; }
    ...
    friend ostream &operator<<(ostream &os, const INTEGER &v) {
        ...
        for (int i = v.z.size() - 2; i >= 0; --i) os << setw(base_digits) << setfill('0') << v.z[i];
    ...
    static vector<int> convert_base(const vector<int> &a, int old_digits, int new_digits) {
        ...
        p[0] = 1;
        for (u64 i = 1; i < p.size(); ++i) p[i] = p[i - 1] * 10;
        vector<int> res;
        i64 cur = 0;
        int cur_digits = 0;
        for (int v: a) {
    ...
    static vector<long long> karatsuba_multiple(const vector<i64> &a, const vector<i64> &b) {
        ...
        int k = n / 2;
        vector<long long> a1(begin(a), begin(a) + k), a2(k + begin(a), end(a));
        vector<long long> b1(begin(b), begin(b) + k), b2(k + begin(b), end(b));
        ...
        for (u64 i = 0; i < a1b1.size(); ++i) r[i] -= a1b1[i];
        for (u64 i = 0; i < a2b2.size(); ++i) r[i] -= a2b2[i];
    ...
    INTEGER operator*(const INTEGER &v) const {
        if (min(z.size(), v.z.size()) < 150) return native_multiple(v);
        INTEGER res;
        res.sign = sign * v.sign;
        vector<int> a6 = convert_base(this->z, base_digits, 6);
        vector<int> b6 = convert_base(v.z, base_digits, 6);
        vector<long long> a(begin(a6), end(a6)), b(begin(b6), end(b6));
        while (a.size() < b.size()) a.push_back(0);
        while (b.size() < a.size()) b.push_back(0);
        while (a.size() & (a.size() - 1)) a.push_back(0), b.push_back(0); // make size power of 2
    ...
    [[maybe_unused]] INTEGER random(u32 digits) {
        string s;
        for (u32 i = 0; i < digits; ++i) s += rand() % 10 + '0';
        return INTEGER(s);
    }
