...
namespace Solver {
    u64 pow_64(u128 a, u64 t, u64 mod) {
        u64 r = 1;
        for (; t; t >>= 1, a = a * a % mod) {
            if (t & 1) r = u128(r) * a % mod;
        }
        return r;
    }
    ...
    void extend_gcd(i128 a, i128 b, i128 &x, i128 &y) {
        if (b == 0) return (void) (x = 1, y = 0);
        extend_gcd(b, a % b, y, x), y -= a / b * x;
    }
    ...
    void init() {
        ...
        auto append = [&](int p) {
            prime[++prime_cnt] = p, _mont_prime[prime_cnt] = mont.init(p), log_p[prime_cnt] = log(p);
        };
    ...
    void main() {
        ...
        while (flag) {
            ...
            for (int x = 1; x <= prime_cnt; ++x) {
                int p = prime[x], a = A % p, inv_a = 1;
                if (a == 0) continue;
                while (a > 1) {
                ...
                for (int su = 0; su < M; su += p) {
                    ...
                    if (su > 0)
                        neg[su - r1] += log_p[x], neg[su - r2] += log_p[x];
                }
            }
